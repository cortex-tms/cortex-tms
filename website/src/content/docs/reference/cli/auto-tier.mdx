...
<Aside type="caution" title="Deprecated in v4.0.0">
  The `auto-tier` command is deprecated and will be removed in a future version. Use [`cortex-tms archive`](/reference/cli/archive/) instead to manually move completed tasks from NEXT-TASKS.md to docs/archive/. The v4.0 pivot focuses on governance validation rather than automatic tier management.
</Aside>
...
- [`cortex-tms status`](/reference/cli/status/) - View project status and tier distribution
- [`cortex-tms validate`](/reference/cli/validate/) - Health checks and documentation validation
- [`cortex-tms init`](/reference/cli/init/) - Initialize TMS (creates files to be tiered)
- [`cortex-tms archive`](/reference/cli/archive/) - Recommended replacement for tier management
...
<Aside type="note" title="Version">
  The `auto-tier` command was introduced in Cortex TMS v3.1.0 (January 2026) and is available in the current release.
