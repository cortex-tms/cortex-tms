...
Apply a governance pack that pre-fills template content with ecosystem-specific
patterns, conventions, and idioms. Independent of `--scope` (which controls which
files are installed).

**Available presets**: `node`

**What the `node` preset pre-fills**:
...
- `docs/core/ARCHITECTURE.md` — common Node layouts (API service, CLI, monorepo)
- `CLAUDE.md` — agent instructions with Node context
- `AGENTS.md` — multi-agent governance with Node-specific prohibitions
- `.github/copilot-instructions.md` — agent rules with Node stack section

**Example**:
```bash
# Node.js project, standard scope, non-interactive
cortex-tms init --preset node --scope standard --force

# Node.js project, minimal setup
cortex-tms init --preset node --minimal --force

# Preview what the Node preset would create
cortex-tms init --preset node --scope standard --dry-run
```

<Aside type="note">
`--preset` and `--scope` are independent. Preset controls content; scope controls the file set. Use them together for full control.
</Aside>
