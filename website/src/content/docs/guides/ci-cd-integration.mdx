...
| `cortex-version` | Cortex TMS version to install (e.g., `latest`, `4.0.1`) | `latest` |
...
jobs:
  validate:
    uses: cortex-tms/cortex-tms/.github/workflows/validate-reusable.yml@v4.0.1
...
jobs:
  validate:
    ...
    steps:
      ...
      - run:
          name: Install cortex-tms
          command: npm install -g cortex-tms@latest
      - run:
          name: Validate Cortex TMS
