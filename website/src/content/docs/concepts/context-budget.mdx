---
title: Documentation Size Management
description: Enforcing file size limits to keep governance docs focused and maintainable
---
...
<Aside type="caution">
  **What happens when docs get too large?**
  - Hard for humans to scan and update
  - AI agents struggle to find relevant information
...
<Aside type="note" title="Why So Strict?">
  HOT files are read **at the start of every AI session**. If `NEXT-TASKS.md` grows to 500 lines, the AI wastes attention on outdated tasks every time.

  **200 lines** keeps the sprint focused and scannable.
</Aside>
...
<Aside type="tip">
  WARM files are read **on demand**, so limits can be higher. But bloat still reduces usefulness—a 2,000-line PATTERNS.md file is hard for AI (and humans) to navigate.
</Aside>
...
<Aside type="tip">
  Duplication wastes space **and** causes drift. Canonical links stay accurate and keep docs focused.
</Aside>
