...
<Terminal
  title="init-output.sh"
  language="bash"
  content={`$ cortex-tms init
...
Users installed Cortex TMS specifically to work with AI agents. Why were we making them manually fill templates when they had Claude Code, Copilot, or Cursor already running?
</Callout>
...
<Terminal
  title="validator.ts"
  language="typescript"
  content={`// Before: Binary (placeholder exists = fail)
...
<Terminal
  title="validate-output.sh"
  language="bash"
  content={`$ cortex-tms validate --verbose
...
<Terminal
  title="ARCHITECTURE.md (AI-generated)"
  language="markdown"
...
<Callout type="tip" title="3-4x Time Savings">
90% accuracy means you spend 5-10 minutes refining, not 30-45 minutes writing from scratch. **Still a 3-4x time savings**.
</Callout>
...
<Terminal
  ...
  language="markdown"
  content={`| \`src/commands/review.ts\` | Guardian AI code review | Anthropic API, LLM client |
| \`src/utils/llm-client.ts\` | LLM API abstraction | Anthropic SDK, retry logic |`}
/>
...
<Terminal
  title="Human-Refined Example"
  language="markdown"
  content={`| \`src/commands/review.ts\` | Guardian AI code review | @anthropic-ai/sdk, streaming responses |
...
<Callout type="tip" title="Universal Compatibility">
The bootstrap prompt uses only standard capabilities (read files, analyze code, write markdown). No API-specific features required.
</Callout>
...
<Callout type="tip" title="Key Lesson">
AI-generated content should be clearly labeled until a human approves it.
</Callout>
...
When users already have the tool (AI agent), give them a workflow, not a new service.
</Callout>
...
<Terminal
  title="state-machine.txt"
  content={`[Placeholder Text] → Error (file is incomplete)
    ↓
  Bootstrap runs
    ↓
...
<Callout type="warning" title="Early Metrics">
These are early metrics. Will update this article with real data after 30-60 days in production.
</Callout>
...
<Terminal
  title="prompt-usage.sh"
  language="bash"
  content={`$ cortex-tms prompt bootstrap
...
<Terminal
  ...
  language="markdown"
  content={`<!-- SETUP NOTE: If you see [placeholder] text above,
...
<Terminal
  title="validator.ts"
  language="typescript"
  content={`async function scanForAIDrafts(filePath: string):
...
<Terminal
  title="after.txt"
  content={`🚀 Quick Start (choose one):
...
<Terminal
  title="bootstrap-auto.sh"
  language="bash"
  content={`$ cortex-tms bootstrap --auto
🔍 Analyzing codebase...
...
<Callout type="warning" title="Decision Point">
If 60%+ of users successfully use the prompt, we may not need the CLI version at all.
</Callout>
...
<Terminal
  title="try-it.sh"
  language="bash"
  content={`# 1. Initialize a project (or use an existing one)
