...
describe("Review E2E - Prerequisites", () => {
  ...
  it("should require PATTERNS.md to exist", async () => {
    ...
    const mentionsPatternsFile =
      result.stdout.includes("PATTERNS.md") ||
...
describe("Review E2E - Safe Mode", () => {
  ...
  it("should support --safe flag", async () => {
    const result = await runCommand("review", ["sample.ts", "--safe"], tempDir);
...
describe("Review E2E - Path Validation", () => {
  ...
  it("should accept valid relative paths", async () => {
    ...
    const result = await runCommand("review", ["src/nested.ts"], tempDir);
