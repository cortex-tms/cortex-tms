 ...
 */
async function getFileTierTag(
  dir: string,
  filename: string,
): Promise<string | null> {
  const content = await readFile(join(dir, filename), "utf-8");
  const match = content.match(
    /<!--\s*@cortex-tms-tier\s+(HOT|WARM|COLD)\s*-->/,
  );
...
describe.skip("Auto-Tier E2E - Command Options", () => {
  ...
  it("should use custom thresholds (--hot, --warm, --cold)", async () => {
    ...
    const customResult = await runCommand(
      "auto-tier",
      ["--dry-run", "--hot", "14"],
      tempDir,
    );
  ...
  it("should use custom --warm and --cold thresholds correctly", async () => {
    ...
    const result1 = await runCommand(
      "auto-tier",
      ["--dry-run", "--hot", "10", "--warm", "40", "--cold", "60"],
      tempDir,
    );
    ...
    const result2 = await runCommand(
      "auto-tier",
      ["--dry-run", "--hot", "10", "--warm", "25", "--cold", "40"],
      tempDir,
    );
...
describe.skip("Auto-Tier E2E - Edge Cases", () => {
  ...
  it("should always tag copilot-instructions.md as HOT (canonical)", async () => {
    ...
    // Even though it's 100 days old, should still be HOT
    const tier = await getFileTierTag(
      tempDir,
      ".github/copilot-instructions.md",
    );
    expect(tier).toBe("HOT");
...
describe.skip("Auto-Tier E2E - Real-World Scenarios", () => {
  ...
  it("should handle typical project with docs hierarchy", async () => {
    ...
    expect(await getFileTierTag(tempDir, "docs/archive/old-design.md")).toBe(
      "COLD",
    );
  });
