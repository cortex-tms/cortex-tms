...
async function runArchive(options: ArchiveOptions): Promise<void> {
  ...
  if (!existsSync(nextTasksPath)) {
    console.log(
      chalk.yellow("⚠️  NEXT-TASKS.md not found in current directory"),
    );
  ...
  try {
    ...
    const completedTaskLines = lines.filter(
      (line) =>
        line.includes("✅") ||
        /\[x\]/i.test(line) ||
    ...
    spinner.succeed(
      `Found ${completedTaskLines.length} completed task markers`,
    );
    ...
    if (options.dryRun) {
      ...
      if (completedTaskLines.length > 5) {
        console.log(
          chalk.gray(`    ... and ${completedTaskLines.length - 5} more\n`),
        );
      }
      console.log(
...
`;
    ...
    console.log(chalk.bold("\n✨ Archive Complete\n"));
    console.log(
      chalk.green(`  ✓ Archived ${completedTaskLines.length} completed tasks`),
    );
    console.log(chalk.gray(`  ✓ Saved to: ${archivePath.replace(cwd, ".")}\n`));
    console.log(chalk.yellow("⚠️  Note: NEXT-TASKS.md was not modified"));
    console.log(
      chalk.gray("    Manually remove completed tasks from NEXT-TASKS.md"),
    );
    console.log(
      chalk.gray(
        "    Or use `cortex-tms validate` to check for cleanup recommendations\n",
      ),
    );
  } catch (error) {
