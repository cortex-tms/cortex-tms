...
**Go-specific prohibitions:**

- Do not use `panic` in production code paths — return errors instead
- Do not use `init()` for complex logic — only trivial package-level variable initialisation
- Do not use global mutable variables (`var` at package scope that mutates after init)
- Do not edit `go.sum` or `go.mod` manually — use `go get` and `go mod tidy`
- Do not use `any` / `interface{}` without a concrete justification in the PR description
- Do not run `go test -run <single-test>` as the only check before commit — always run `go test ./...`
